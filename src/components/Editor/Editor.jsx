...
export default function Editor({ title, setTitle,
    ...
    const heroes = ['menino', 'menina', 'homem', 'mulher', 'idoso', 'idosa', 'animal', 'objeto', 'lugar'];

    const [conflict, setConflict] = useState('homem versus homem');
    const conflicts = ['indivíduo contra indivíduo', 'indivíduo contra natureza', 'indivíduo contra si mesmo',
        'indivíduo contra destino/sorte', 'indivíduo contra máquina', 'indivíduo contra sistema',
        'indivíduo contra passado'];

    const [genre, setGenre] = useState('terror');
    const genres = ['terror', 'suspense', 'drama', 'comédia', 'romance', 'aventura', 'policial', 'fantasia'];

    const [ambient, setAmbient] = useState('escola');
    const ambients = ['escola', 'empresa', 'cidade pequena', 'cidade grande', 'comunidade rural', 'presídio',
        'hospital', 'floresta'];

    const [time, setTime] = useState('contemporâneo');
    const times = ['pré-histórico', 'medieval', 'contemporâneo', 'futurístico', 'pós-apocalíptico'];
    ...
    return (
        <div className="editor">
            ...
            <select value={conflict} onChange={(e) => setConflict(e.target.value)}>
                {conflicts.map((value) => (<option key={value} value={value}>{value}</option>))}
            </select>
            ...
            <p>{`Escreva uma história com um(a) protagonista "${hero}"  com umconflito do tipo
            "${conflict}" do gênero "${genre}" ambientada em um(a) "${ambient}" no tempo "${time}" para o 
            público "${audience}". ${additional}.`}</p>
            ...
            <textarea className="description" value={description} onChange={(e) =>
